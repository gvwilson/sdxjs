...
a [%g source_map "source map" %] that keeps track of
...
[%inc debugger-trace.js %]

Let's try it on a program that adds the numbers in an array:

[%inc sum-source-map.json %]
[%inc sum-source-map-trace.out %]
...
[%inc debugger-interactive.js mark=next %]

while this one prints the value of a variable:


[%inc debugger-interactive.js mark=print %]
...
[%inc test/test-expect.js mark=tests %]
...
[%inc test/test-expect.js mark=setup %]
