...
That's OK for [%g exploratory_programming "exploratory programming" %],
...
It uses the [%g singleton_pattern "Singleton" %] [%g design_pattern "design pattern" %]
...
1.  Node [%g caching "caches" %] modules
...
as a [%g side_effect "side effect" %] via calls to `hope.test`
...
Our [%g test_runner "test runner" %] is now complete,
...
as this happens
so we must specify the [%g lifecycle "lifecycle" %] of the loaded modules quite carefully.
