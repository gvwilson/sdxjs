...
A [%g module_bundler "module bundler" %]
...
[%figure slug=module-bundler-bundling img=bundling.svg alt="Bundling modules" caption="Combining multiple modules into one." %]

Bundling requires an [%g entry_point "entry point" %],
i.e.,
...
[%figure slug=module-bundler-complicated img=complicated.svg alt="Module bundler dependencies" caption="Dependencies in large module bundler test case." %]
...
[%inc expected-full.out %]

We do not handle circular dependencies
because `require` itself doesn't ([%x loader %]).
...
we need to find the [%g transitive_closure "transitive closure" %] of the entry point's dependencies,
i.e.,
...
[%figure slug=module-bundler-transitive-closure img=transitive-closure.svg alt="Implementing transitive closure" caption="Implementing transitive closure using two sets." %]
...
As with the module loader in [%x loader %],
...
[%inc transitive-closure-only.js %]
...
[%figure slug=module-bundler-structure img=structure.svg alt="Data structure for modules" caption="Data structure used to map names to absolute paths." %]
...
We do this using the same method we used in [%x loader %]:
...
[%inc sanity-check-unwrapped.js %]

The wrapped version will look like this:


[%inc sanity-check-wrapped.js %]
...
[%figure slug=module-bundler-head-tail img=head-tail.svg alt="Assembling runnable code" caption="Assembling fragments and modules to create a bundle." %]

We can test that this works in our two-file case:

[%inc test-combine-files.js %]
[%inc test-combine-files-simple.js %]

and then load the result and call `initialize`:


[%inc show-combine-files-simple.out %]
...
As in [%x loader %] we solve this problem using closures.
...
[%figure slug=module-bundler-returning-functions img=returning-functions.svg alt="Functions returning functions returning functions" caption="A function that returns functions that return functions." %]

We also need a third structure:
a cache for the modules we've already loaded.
Putting it all together we have:

[%inc create-bundle.js %]
...
except the promises in [%x async %].
However,
...
[%inc test-create-bundle-single.sh %]
[%inc bundle-single.js %]

and then we run it:


[%inc test-bundle-single.out %]
...
[%inc bundle-simple.js %]
[%inc test-bundle-simple.out %]

and so does our most complicated test with `main` and four other files:


[%inc test-bundle-full.out %]
...
Suppose we wanted to compress the files being stored by the file backup system in [%x backup %]
