...
This chapter will use what we learned in [%x codegen %] about parsing source code
...
[%inc extract-comments-subset.js %]
...
[%g line_comment "Line comments" %] cannot span multiple lines;
...
[%g block_comment "Block comments" %],
on the other hand,
...
The [%g doc_comment "doc comments" %] for function definitions look like this:

[%inc example-plain.js %]
...
[%inc process-plain.js mark=main %]
...
[%inc process-plain.js mark=getAllComments %]

Extracting the comments from a single file is done as before:

[%inc process-plain.js mark=extractComments %]
...
[%inc process-plain.js mark=removePrefix %]

One thing that isn't in this file (because we're going to use it in later versions)
is the function `slugify`.
A [%g slug "slug" %] is a short string that identifies a header or a web page;
...
[%inc process-plain.sh %]
[%inc process-plain.html %]
...
[%inc find-following.js mark=main %]

The recursive search is straightforward as well---we delete line numbers from the target set
and add nodes to the [%g accumulator "accumulator" %] as we find matches:

[%inc find-following.js mark=findFollowing %]

Finally,
we use a function called `condense` to get the name we want out of the AST:

[%inc find-following.js mark=condense %]
