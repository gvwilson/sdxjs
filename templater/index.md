...
2.  Create a mini-language with its own commands like [Jekyll][jekyll]
    (which is used by [GitHub Pages][github_pages]).
...
[%figure slug=page-templates-options img=options.svg alt="Three options for page templates" caption="Three different ways to implement page templating." %]
...
to conform with [the HTML5 specification][html5_data_attributes],
...
Each [%g stack_frame "stack frame" %] is an object with names and values;
...
[%figure slug=page-templates-stack img=stack.svg alt="Variable stack" caption="Using a stack to manage variables." %]

## How do we handle nodes? {: #page-templates-nodes}

HTML pages have a nested structure,
so we will process them using
the [%g visitor_pattern "Visitor" %] design pattern.
`Visitor`'s constructor takes the [%g root_tree "root node" %] of the DOM tree as an argument and saves it.
...
[%figure slug=page-templates-visitor img=visitor.svg alt="The Visitor pattern" caption="Using the Visitor pattern to evaluate a page template." %]
...
but [%g bare_object "bare objects" %] are common and useful in JavaScript
...
[%inc input-static-text.html %]
[%inc static-text.sh %]
[%inc output-static-text.html %]

[%figure slug=page-templates-output-static-text img=output-static-text.svg alt="Generating static text" caption="Static text generated by page templates." %]

Good.
Now, does the expander handle constants ([%f page-templates-output-single-constant %])?

[%inc input-single-constant.html %]
[%inc output-single-constant.html %]

[%figure slug=page-templates-output-single-constant img=output-single-constant.svg alt="Generating a single constant" caption="A single constant generated by page templates." %]

What about a single variable ([%f page-templates-output-single-variable %])?

[%inc input-single-variable.html %]
[%inc output-single-variable.html %]

[%figure slug=page-templates-output-single-variable img=output-single-variable.svg alt="Generating a single variable" caption="A single variable generated by page templates." %]
...
[%inc input-multiple-variables.html %]
[%inc output-multiple-variables.html %]

[%figure slug=page-templates-output-multiple-variables img=output-multiple-variables.svg alt="Generating multiple variables" caption="Multiple variables generated by page templates." %]
...
[%inc input-conditional.html %]
[%inc output-conditional.html %]

[%figure slug=page-templates-output-conditional img=output-conditional.svg alt="Generating conditional text" caption="Conditional text generated by page templates." %]
...
[%figure slug=page-templates-output-loop img=output-loop.svg alt="Generating text with a loop" caption="Repeated text generated with a loop by page templates." %]
...
[%figure slug=page-templates-comprehension img=comprehension.svg alt="Comprehension curves" caption="Novice and expert comprehension curves." %]
