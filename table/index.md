...
to the [%g data_frame "data frames" %] in
R's [tidyverse][tidyverse] packages,
[Python's][python] [Pandas][pandas] library,
or the [DataForge][data_forge] library for JavaScript [%b Davis2018 %].

[%figure slug=data-table-conceptual img=conceptual.svg alt="Data table structure" caption="The structure of a data table." %]

The key operations on data tables are those provided by [%g sql "SQL" %]:
...
One way to store a table is [%g row_major "row-major" %] order,
in which the values in each row are stored together in memory.
This is sometimes also called [%g heterogeneous "heterogeneous" %] storage
...
Another option is [%g column_major "column-major" %]
or [%g homogeneous "homogeneous" %] order,
...
All of our implementations will treat each data table as [%g immutable "immutable" %]:
...
[%inc table-performance.js mark=operate-rows %]
...
[%inc table-performance.js mark=operate-cols %]
...
we can build a [%g test_harness "test harness" %] to run those operations
...
[%inc table-performance.js mark=main %]
...
[%inc table-performance.js mark=measure %]
...
[%inc table-performance-10000-30-03.out %]

And what if we keep the table size the same but use a 10:1 filter/select ratio?


[%inc table-performance-10000-30-10.out %]

<div id="t:data-table-performance" data-caption="Relative performance of operations on row-major and column-major data tables." markdown="1">
...
Two simple text-based schemes are row-oriented and column-oriented JSON---basically,
just printing the data structures we have.
Let's run the 10,000×30 test:

[%inc storage-performance-10000-30.out %]
...
[%inc packed-rows.js mark=packed %]
[%inc packed-rows-10000-30.out %]
...
JavaScript stores values in [%g tagged_data "tagged" %] data structures:
...
[%inc packed-cols.js mark=binary %]
[%inc packed-cols-10000-30.out %]
