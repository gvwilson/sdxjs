...
is that many operations in JavaScript are [%g asynchronous "asynchronous" %].
...
[Node][nodejs] implements [%g filesystem "filesystem" %] operations the same way.
...
<div id="t:systems-programming-times" data-caption="Computer operation times at human scale." markdown="1">
...
Early JavaScript programs used [%g callback "callback functions" %] to describe asynchronous operations,
...
while [%x async %] shows how promises and `async`/`await` work.
...
We tell the program what to list using [%g command_line_argument "command-line arguments" %],
...
this reduces the [%g cognitive_load "cognitive load" %] on people reading the program.
Finally,
...
JavaScript uses a [%g single_threaded "single-threaded" %] programming model:
...
Those parameters defined a standard [%g protocol "protocol" %]
...
[%inc list-dir-function-defined.js %]
...
as an [%g anonymous_function "anonymous function" %].
...
[%inc list-dir-function-anonymous.js %]
...
(To [%g globbing "glob" %] (short for "global") is an old Unix term for matching a set of files by name.)
...
We can get rid of them by [%g filter "filtering" %] the list that `glob` returns:
...
[%inc glob-get-then-filter-idiomatic.js %]
...
[%inc glob-filter-with-options.js %]
...
[%inc glob-with-source-directory.js %]

This program uses [%g string_interpolation "string interpolation" %]
...
[%inc glob-with-dest-directory.js %]

This program uses [%g destructuring_assignment "destructuring assignment" %]
...
[%inc glob-ensure-output-directory.js %]
...
`fs.stat` returns [an object with methods that can answer common questions][node_fs_stats].

Here's the final version of our file copying program:

[%inc copy-file-filtered.js %]

It works,
but four levels of asynchronous callbacks is hard for humans to understand.
[%x async %] will introduce a pair of tools
...
[%inc x-trace-callback/trace.js %]

### Tracing anonymous callbacks 

In what order does the program below print messages?

[%inc x-trace-anonymous/trace.js %]
