...
JavaScript's developers added [%g promise "promises" %] to the language in 2015
...
Our explanation was inspired by [Trey Huffine's][huffine_trey] [tutorial][huffine_promises],
and we encourage you to read that as well.

## How can we manage asynchronous execution? {: #async-programming-manage}

JavaScript is built around an [%g event_loop "event loop" %].
...
[%g non_blocking_execution "non-blocking function" %]
...
we will allow users to [%g method_chaining "chain" %] as many `then`s as they want,
but only allow one `catch`.

## How can we chain operations together? {: #async-programming-fluent}

A [%g fluent_interface "fluent interface" %]
...
and so on.
If any of them fail and throw an [%g exception "exception" %],
...
A [%g character_encoding "character encoding" %]
specifies how characters are stored as bytes.
The most widely used is [%g utf_8 "UTF-8" %],
...
[%inc count-lines-print-filenames.js %]
...
The latest versions of JavaScript provide two new keywords `async` and `await`
...
we will see how this works in [%x codegen %].
...
[%inc count-lines-with-stat-async.js mark=recycle %]
...
[%inc count-lines-with-stat-async.js mark=main %]
...
[%inc x-multiple-catch/example.js %]

When the code is run it produces:


[%inc x-multiple-catch/example.txt %]
...
[%inc x-catch-then/example.js %]

When the code is run it produces:


[%inc x-catch-then/example.txt %]
