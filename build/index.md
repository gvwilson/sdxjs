...
Suppose we are using a page templating system to create a website ([%x templater %]).
...
For example,
programs in [%g compiled_language "compiled languages" %]
like C and Java
...
A [%g build_manager "build manager" %] takes a description of what depends on what,
...
they are also useful for programs written in [%g interpreted_language "interpreted languages" %]
like JavaScript
when we want to bundle multiple modules into a single loadable file ([%x bundler %])
or re-create documentation from source code ([%x docgen %]).
In this chapter we will create a simple build manager
based on [Make][gnu_make], [Bajel][bajel], [Jake][jake],
...
-   a [%g build_target "target" %], which is the file to be updated;

-   some [%g dependency "dependencies" %], which are the things that file depends on;
    and

-   a [%g build_recipe "recipe" %] that specifies how to update the target
...
so we start by writing a simple [%g driver "driver" %] that loads a JavaScript source file,
...
We use the `import` function to dynamically load files in [%x test %] as well.
...
Just as we built a generic `Visitor` class in [%x templater %],
...
And as we first saw in [%x async %],
...
Since we already looked at hashing in [%x backup %],
we will use the timestamp approach here.
And instead of using a mock filesystem as we did in [%x backup %],
...
Instead,
we want to be able to write generic [%g build_rule "build rules" %] that say,
...
Make provides [%g automatic_variable "automatic variables" %]
with names like `$<` and `$@`
...
and as [%g runnable_documentation "runnable documentation" %]
