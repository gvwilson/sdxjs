...
Programs that do this are often called [%g linter "linters" %]
...
Just as the world doesn't need more file format ([%x parser %])
...
[Standard JS][standard_js] may not do everything exactly the way you want,
...
and produces an [%g abstract_syntax_tree "abstract syntax tree" %] (AST)
...
we need to [%g walk_tree "walk the tree" %],
i.e.,
to visit each node in turn.
The [`acorn-walk`][acorn_walk] library will do this for us
using the Visitor design pattern we first saw in [%x templater %].
...
[%inc walker-class.js mark=walker %]

The code we need to use it is:


[%inc walker-class.js omit=walker %]

and its output is:


[%inc walker-class.out %]
...
the [%g iterator_pattern "Iterator" %] design pattern.
...
[%g generator_function "generator functions" %].
...
[%inc generator-vowels-for.js mark=loop %]
...
[%inc generator-count.js mark=generator %]

and the program that uses it is:


[%inc generator-count.js mark=main %]

When we run it with our usual test program as input, we get:

[%inc generator-count.out %]
...
And finally,
we can print a [%g markdown "Markdown" %]-formatted table
showing which methods are defined in which class:

[%inc run-find-methods.raw.out %]
...
the virtual machine we will build in [%x vm %],
...
1.  Write a generator that takes a two-dimensional table represented as an array of arrays
    and returns the values in column-major order.

2.  Write another generator that takes a similar table
    and returns the values in row-major order.
