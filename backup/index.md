...
please see [Mary Rose Cook's][cook_mary_rose] excellent [Gitlet][gitlet] project.
...
A hash function always produces the same [%g hash_code "hash code" %] for a given input.
A [%g cryptographic_hash_function "cryptographic hash function" %]
...
We will therefore use a library to calculate 160-bit [%g sha_1 "SHA-1" %] hashes for our files.
...
we just want hashes that are random to make [%g collision "collision" %] extremely unlikely.
...
and doing a few calculations with [Wolfram Alpha][wolfram_alpha],
...
a [%g streaming_api "streaming" %] [%g api "API" %]
...
by providing a [%g handler "handler" %] for the "finish" event.
...
We can build the tools we need to do this using promises ([%x async %]).
...
[%inc hash-existing-promise.js mark=main %]
...
[%inc hash-existing-promise.js mark=helpers %]
...
[%inc hash-existing-promise.js mark=hashPath %]
...
[%inc hash-existing-async.js mark=main %]
...
[%inc test/test-find.js %]

and here is Mocha's report:


[%inc test-check-filesystem.out %]
...
A better approach is to use a [%g mock_object "mock object" %]
...
[%inc test/test-find-mock.js omit=tests %]
...
[%inc test/test-backup.js mark=fixtures %]

and then run some tests:


[%inc test/test-backup.js mark=tests %]
[%inc test-backup.out %]
...
is by thinking about testability [%b Feathers2004 %].
