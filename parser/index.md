...
In [%x glob %] we created regular expressions by constructing objects.
...
In other words, we need to write a [%g parser "parser" %].

<div id="t:regex-parser-grammar-codes" data-caption="Regular expression grammar." markdown="1">
| Meaning | Character |
| ------- | --------- |
...
rather than instances of the regular expression classes from [%x glob %];
...
A [%g token "token" %] is an atom of text,
...
[%inc tokenizer-collapse.js mark=combine %]
...
[%inc test/test-tokenizer.js omit=omit %]
[%inc tokenizer-test.out %]
...
If we have more operators with different [%g precedence "precedences" %]
we should switch to the [shunting-yard algorithm][shunting_yard_algorithm],
...
For example,
every regular expression corresponds to a [%g fsm "finite state machine" %] (FSM)
...
and [attempting to do so is a sin][stack_overflow_html_regex].
If you add a stack to the system you can process a much richer set of languages,
and if you add two stacks you have something equivalent to a [%g turing_machine "Turing Machine" %]
