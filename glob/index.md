...
We have been globbing to match filenames against patterns since [%x sysprog %].
This lesson will explore how that works
by building a simple version of the [%g regular_expression "regular expressions" %]
used to match text in everything from editor and shell commands to web scrapers.
Our approach is inspired by [Brian Kernighan's][kernighan_brian] entry
in [%b Oram2007 %].

Regular expressions have inspired pattern matching for many other kinds of data,
such as [%g query_selector "query selectors" %] for HTML.
...
[%figure slug=pattern-matching-dom-tree img=dom-tree.svg alt="The Document Object Model" caption="Representing an HTML document as a tree." %]
...
<div id="t:pattern-matching-supported" data-caption="Supported patterns." markdown="1">
| Meaning | Selector |
| ------- | -------- |
...
[%figure slug=pattern-matching-query-selectors img=query-selectors.svg alt="Matching query selectors" caption="Matching a simple set of query selectors." %]

[%inc simple-selectors.js omit=skip %]
...
This algorithm is called [%g depth_first "depth-first search" %]:
...
[%inc simple-selectors.js mark=firstChild %]

and on the function `matchHere` which compares a node against a selector:


[%inc simple-selectors.js mark=matchHere %]
...
[%inc simple-selectors-test.js mark=tests %]
...
[%inc simple-selectors-test.js mark=main %]
...
[%inc simple-selectors-test.js mark=getText %]


When we run our program it produces this result:

[%inc simple-selectors-test.out %]
...
Doing this is called [%g tdd "test-driven development" %], or TDD;
while research doesn't support the claim that
it makes programmers more productive [%b Fucci2016 Fucci2017 %],
we find it helps prevent [%g scope_creep "scope creep" %] when writing lessons.
...
<div id="t:pattern-matching-cases" data-caption="Pattern matching cases." markdown="1">
| Meaning | Character |
| ------- | --------- |
...
[%inc simple-regex.js mark=match %]

`matchHere` does the matching and recursing:

[%inc simple-regex.js mark=matchHere %]

Once again,
we use a table of test cases and expected results to test it:

[%inc simple-regex.js mark=tests %]
[%inc simple-regex.out %]
...
[%inc regex-initial/regex-complete.js %]
...
[%inc regex-initial/regex-base.js %]
...
Our design makes use of [%g polymorphism "polymorphism" %],
...
Polymorphism reduces the [%g coupling "coupling" %] between different parts of our program,
...
[%inc regex-initial/regex-lit.js %]
...
[%inc regex-beginning/regex-lit.js %]

Some tests now pass, others still fail as expected:

[%inc regex-beginning.out %]
...
The `/*/` is [%g greedy_algorithm "greedy" %]: it matches as much as it can
(which is also called [%g eager_matching "eager matching" %]).
As a result,
...
[%figure slug=pattern-matching-greedy-failure img=greedy-failure.svg alt="Overly-greedy matching fails" caption="Why overly greedy matching doesn't work." %]
...
[%figure slug=pattern-matching-rest img=rest.svg alt="Matching the rest of the pattern" caption="Using `rest` to match the remainder of a pattern." %]

This design means we can get rid of `RegexSeq`,
but it does make our tests a little harder to read:

[%inc regex-recursive/regex-complete.js %]

Here's how this works for matching a literal expression:

[%inc regex-recursive/regex-lit.js %]
...
[%inc regex-recursive/regex-any.js %]

With these classes in place,
our tests all pass:

[%inc regex-recursive.out %]
...
and the [%g chain_of_responsibility_pattern "Chain of Responsibility" %] design pattern.
...
The regular expressions we have seen so far are eager:
