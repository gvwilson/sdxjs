...
based on [Matt Brubeck's][brubeck_matt] [tutorial][browser_tutorial]
...
as [Mike Hoye][hoye_mike] has [repeatedly observed][punching_holes],
...
[%inc test/test-easy-mode.js %]
[%inc test-easy-mode.out %]
...
[%inc render.js mark=draw %]

To teach each kind of cell how to render itself,
we have to derive a new class from each of the ones we have
and give the new class a `render` method with the same [%g signature "signature" %]:

[%inc rendered.js mark=keep %]
...
[%inc test/test-rendered.js mark=large %]
...
the [%g liskov_substitution_principle "Liskov Substitution Principle" %],
which states that
...
[%g design_by_contract "design by contract" %].
...
[%inc parse.js omit=skip %]

while the two functions that do most of the work are:


[%inc parse.js mark=makenode %]

and:


[%inc parse.js mark=makeopening %]
...
[%inc micro-css.js mark=css %]

An ID rule's query selector is written as `#name`
and matches HTML like `<tag id="name">...</tag>` (where `tag` is `row` or `col`):

[%inc micro-css.js mark=id %]
...
[%inc test/test-styled.js mark=test %]
