...
[%x interp %] showed how to use `eval` to load code dynamically.
...
We therefore need a way to [%g encapsulate "encapsulate" %] the contents of what we're loading.
...
We can do this using [%g closure "closures" %].
...
and return that [%g inner_function "inner function" %],
...
an [%g iife "immediately-invoked function expression" %] (IIFE).
...
We can visualize the network of who requires whom as a [%g directed_graph "directed graph" %]:
if X requires Y,
we draw an arrow from X to Y.
Unlike the directed *acyclic* graphs we met in [%x build %],
though,
these graphs can contain cycles:
we say a [%g circular_dependency "circular dependency" %] exists
if X depends on Y and Y depends on X
either directly or indirectly.
This may seem nonsensical,
but can easily arise with [%g plugin_architecture "plugin architectures" %]:
...
Circular dependencies work in [Python][python],
but only sort of.
Let's create two files called `major.py` and `minor.py`:

[%inc checking/major.py %]
[%inc checking/minor.py %]
...
[%inc checking/major.js %]
[%inc checking/minor.js %]

It fails on the command line:


[%inc checking/js-command-line.out %]

and also fails in the interactive interpreter
(which is more consistent):


[%inc checking/js-interactive.out %]
...
because someone might create an [%g alias "alias" %]
...
[%inc x-refactoring-circularity/main.js %]

and `plugin.js` contains this:


[%inc x-refactoring-circularity/plugin.js %]
